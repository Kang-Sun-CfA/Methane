...
windowlist(1).vEnd = 1380;
windowlist(1).gas_cell = {'O2','O4','H2O','CO2'};

windowlist(2).vStart = 1590;
windowlist(2).vEnd = 1710;
windowlist(2).gas_cell = {'CH4','H2O','CO2'};

windowlist(3).vStart = 2230;
windowlist(3).vEnd = 2400;
windowlist(3).gas_cell = {'CH4','H2O','N2O','CO'};
...
for iangle = 1:1%length(SZA_array)
    ...
    for ialb = 1:length(ReflSpectra_array)
        ...
        for iaer = 1:length(aername_array)
        ...
        for iwin = 1:length(windowlist)
            ...
            inp.dv_calc = 0.02;
            inp.fn_extra = [aername_array{iaer},'_',ReflName];
